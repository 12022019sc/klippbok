...
/**
 * AdvancedPanel wraps the existing Ingest/Scan/Extract tab content
 * in a collapsible section. Lift-and-shift from original VideoPage.
 */
export default function AdvancedPanel() {
  ...
  return (
    <div className="advanced-panel">
      ...
      >
        <span>{isOpen ? '\u25BC' : '\u25B6'} Advanced Video Pipeline</span>
      </button>
            ...
            <div className="video-tab-panel">
              ...
              {showAdvanced && (
                <div className="video-advanced">
                  <div className="video-field">
                    ...
                    <input type="number" className="video-input-sm" value={fps} min={1} max={60} onChange={(e) => setFps(Number(e.target.value))} disabled={ingestInProgress} />
                  </div>
                  <div className="video-field">
                    <label className="video-label">Resolution (height px)</label>
                    <input type="number" className="video-input-sm" value={resolution} min={360} max={4320} step={180} onChange={(e) => setResolution(Number(e.target.value))} disabled={ingestInProgress} />
                  </div>
                  <div className="video-field">
                    <label className="video-label">Scene threshold</label>
                    <input type="number" className="video-input-sm" value={threshold} min={1} max={100} step={0.5} onChange={(e) => setThreshold(Number(e.target.value))} disabled={ingestInProgress} />
                  ...
                  <div className="video-field">
                    ...
                    <input type="number" className="video-input-sm" value={maxFrames} min={1} placeholder="Unlimited" onChange={(e) => setMaxFrames(e.target.value === '' ? '' : Number(e.target.value))} disabled={ingestInProgress} />
                  </div>
                </div>
              )}

              <div className="video-actions">
                <button className="video-btn-primary" onClick={() => void handleStartIngest()} disabled={ingestInProgress || isStartingIngest}>
                  {isStartingIngest ? 'Starting...' : ingestInProgress ? 'Ingesting...' : 'Start Ingest'}
                </button>
                {ingestInProgress && (
                  <button className="video-btn-cancel" onClick={() => void handleCancelIngest()}>Cancel</button>
                )}
              </div>
              ...
              {ingestInProgress && (
                <div className="video-progress">
                  ...
                  <div className="video-progress-track">
                    <div className="video-progress-fill" style={{ width: `${ingestPercent()}%` }} />
                  </div>
                  <p className="video-progress-message">{ingestProgress.message}</p>
                  {ingestProgress.total > 0 && (
                    <p className="video-progress-count">{ingestProgress.current} / {ingestProgress.total}</p>
                  )}
                </div>
              )}
              ...
              {ingestClipCount !== null && !ingestInProgress && (
                <div className="video-success">
                  ...
                  <button className="video-btn-link" onClick={() => { setActiveTab('scan'); void fetchScan() }}>
                    View in Scan tab
                  </button>
                </div>
              )}
          ...
          {activeTab === 'scan' && (
            <div className="video-tab-panel">
              <div className="video-scan-header">
                ...
                <button className="video-btn-secondary" onClick={() => { scanLoadedRef.current = false; void fetchScan() }} disabled={scanLoading}>
                  {scanLoading ? 'Scanning...' : 'Refresh'}
                </button>
              </div>
              {scanError && <div className="video-error">Error: {scanError}</div>}
              {!scanLoading && !scanError && scanResults.length === 0 && (
                <div className="video-empty">No video clips found in project. Use Ingest to process videos.</div>
              )}
              {scanResults.length > 0 && (
                <div className="video-table-wrapper">
                  <table className="video-table">
                    <thead>
                      <tr>
                        <th>Filename</th><th>Resolution</th><th>FPS</th><th>Duration</th><th>Codec</th><th>Frames</th><th>Issues</th>
                      </tr>
                    </thead>
                    <tbody>
          ...
          {activeTab === 'extract' && (
            <div className="video-tab-panel">
              <div className="video-field">
                ...
                <input type="number" className="video-input-sm" value={framesPerClip} min={1} max={50} onChange={(e) => setFramesPerClip(Number(e.target.value))} disabled={extractInProgress} />
                <span className="video-hint">Number of reference frames to extract from each clip</span>
              </div>
              <div className="video-actions">
                <button className="video-btn-primary" onClick={() => void handleStartExtract()} disabled={extractInProgress || isStartingExtract}>
                  {isStartingExtract ? 'Starting...' : extractInProgress ? 'Extracting...' : 'Extract Frames'}
                </button>
                {extractInProgress && (
                  <button className="video-btn-cancel" onClick={() => void handleCancelExtract()}>Cancel</button>
                )}
              </div>
              {extractInProgress && (
                <div className="video-progress">
                  ...
                  <div className="video-progress-track">
                    <div className="video-progress-fill" style={{ width: `${extractPercent()}%` }} />
                  </div>
                  <p className="video-progress-message">{extractProgress.message}</p>
                  {extractProgress.total > 0 && (
                    <p className="video-progress-count">{extractProgress.current} / {extractProgress.total}</p>
                  )}
                </div>
              )}
                  ...
                  <div className="video-clips-thumbnails">
                    {extractedClips.map((clip) => (
                      <div key={clip.id} className="video-clip-card">
                        <img src={clip.thumbnail_url} alt={clip.relative_path} className="video-clip-thumb" />
                        <p className="video-clip-name">{clip.relative_path.split('/').pop()}</p>
